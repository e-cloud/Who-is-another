...
class SubmitlogCrawlTask implements Callable<List<SubmitLog>> {
	...
	static Queue<String> urlQueue = null;

	/**
		 * 
		 */
	public SubmitlogCrawlTask(String authorID) {
		...
		try {
			while (!urlQueue.isEmpty()) {
				...
				if (url != null) {
					...
					int first;
					first = SubmitLogsParser.parse(data, submitLogs);
					// author.add(submitLogs);
					synchronized (urlQueue) {
						if (first > 0) {
							String pushurl = DataPreprocessor
									.getRealTimeStatusUrl(
											submitLogs.get(
													submitLogs.size() - 1)
													.getPid(), authorID, first);
